...
async function getCache(RED) {
    let settings = RED.settings.get('dt-state');
...
async function setCache(RED, cache) {
    ...
    await RED.settings.set('dt-state', settings);
}
...
async function removeCachedFile(RED, cacheEntry) {
    try {
        let cacheFile = path.join(RED.settings.userDir, 'dt-state', cacheEntry.file);
...
async function cacheRendering(RED, smcatStr, data, ttl) {
    ...
    let file = `${hash}.json`;
    let filedir = path.join(RED.settings.userDir, 'dt-state');
    ...
    try {
        ...
        console.error(`Error writing dt-state cache file ${cacheFile}: ${err}`);
        return false;
    }
    ...
    if( cacheHit ) {
        ...
        try {
            ...
            if( cacheHit.hasOwnProperty('file') ) {
                    cacheFile = path.join( RED.settings.userDir, 'dt-state',  cacheHit.file );
            ...
            console.log(`Invalid dt-state cache entry: ${err}. Deleting from cache...`);
            await setCache(RED, removeCacheEntry(cache, cacheHit));
            // Remove cached file if it exists
            await removeCachedFile(RED, cacheHit);
        }
