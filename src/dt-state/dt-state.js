module.exports = function (RED) {
	...
	var smcat     = require('../dt-state/src/smcat-render');
	var immutable = require('immutable');
	xstate.smcat  = require('../dt-state/src/xstate-smcat');
	RED.dtstate  = {};
	RED.dtstate.settings = require('../dt-state/src/dtstate-settings');
...
`
	...
	function restartMachine(node) {
		...
		let transitionFcn = (state) => {
			...
			if( activeId == node.id ) {
				RED.comms.publish("dtstate_transition",{
					type: 'transition',
					id: node.id,
	...
	function StateMachineNode (config) {
		...
		RED.comms.publish('dt-state', { type: 'add', data: nodeinfo });
		...
		try {
			this.initscript = vm.createScript(getFunctionText(this), {
				filename: 'DTSTATE node:'+this.id+(this.name?' ['+this.name+']':''),
				displayErrors: true
			});
	...
	RED.nodes.registerType('dt-state', StateMachineNode);

	// Do one time init tasks
	smcat.init(RED);

	// Initialize settings
	RED.dtstate.settings.init(RED);

	RED.httpAdmin.get("/dtstate/:method", RED.auth.needsPermission("dtstate.read"), function(req,res) {
		switch(req.params.method) {
			case 'getnodes':
				try {
					...
					res.sendStatus(500);
					console.error(`dtstate: GET Command failed: ${err.toString()}`);
				}
				break;
			case 'settings':
				try {
					let property = req.query.property;
					let value = RED.dtstate.settings.get(property);
					if( value !== null )
						res.status(200).send({ [property]: value });
					else {
						res.sendStatus(404);
						console.error(`dtstate: No setting named ${property} is available`);
					}
				} catch(err) {
					res.sendStatus(500);
					console.error(`dtstate: GET Command failed: ${err.toString()}`);
			...
			default:
				res.sendStatus(404);
				console.error(`dtstate: Invalid method: ${req.params.method}`);
				break;
		}
	});

	RED.httpAdmin.post("/dtstate/:method", RED.auth.needsPermission("dtstate.write"), function(req,res) {
		switch(req.params.method) {
			case 'settings':
				try {
					RED.dtstate.settings.set(req.body.property, req.body.value ).then( () => {
						res.status(200).send("success");
					}).catch( (err) => { throw(err); });
				} catch(err) {
					res.sendStatus(500);
					console.error(`dtstate: POST Command for method ${req.params.method} failed: ${err.toString()}`);
				}
				break;
			default:
				res.sendStatus(404);
	...
	RED.httpAdmin.post("/dtstate/:id/:method", RED.auth.needsPermission("dtstate.write"), function(req,res) {
		...
		if (node != null) {
						...
						(async () => {
							try {
								...
								let output = await smcat.render(smcat_machine, {
									timeoutMs: Number.parseInt(RED.dtstate.settings.get('renderTimeoutMs')),
									renderer: RED.dtstate.settings.get('renderer'),
									logOutput: true,
			...
			console.error(`dtstate: node with id ${req.params.id} does not exist for method ${req.params.method}.`)
			res.sendStatus(404);
		}
	});
};
